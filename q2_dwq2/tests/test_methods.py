...
from q2_dwq2._methods import (nw_align, local_alignment_search, chunk_sequences,
                              collate_las_reports)
...
class ChunkSequencesTests(TestPluginBase):
    package = 'q2_dwq2.tests'

    def _flatten_chunks(self, chunks):
        result = []
        for k, dna_iter in chunks.items():
            result.extend(list(dna_iter))
        return result

    def test_one_seq_per_chunk(self):
        sequences = [
        ...
        # confirm the expected number of chunks are returned
        observed = chunk_sequences(sequences, chunk_size=1)
        self.assertEqual(len(observed), 6)
        # confirm that all input sequences are contained in the
        # chunked sequences
        self.assertEqual(self._flatten_chunks(observed), sequences)
    ...
    def test_one_chunk(self):
        ...
        observed = chunk_sequences(sequences, chunk_size=6)
        ...
        observed = chunk_sequences(sequences, chunk_size=2)
        self.assertEqual(len(observed), 1)
        self.assertEqual(self._flatten_chunks(observed), sequences)

    def test_multiple_seqs_multiple_chunks(self):
        sequences = [
            DNA('ACACTCACCACCCAATTGCT', metadata={'id': 'r1'}),
            DNA('ACACTCTCCACCCATTTGCT', metadata={'id': 'r2'}),
            DNA('ACACTCTCCAGCCATTTGCT', metadata={'id': 'r3'}),
        ...
        observed = chunk_sequences(sequences, chunk_size=3)
        self.assertEqual(len(observed), 2)
        self.assertEqual(self._flatten_chunks(observed), sequences)

        observed = chunk_sequences(sequences, chunk_size=4)
        self.assertEqual(len(observed), 2)
        self.assertEqual(self._flatten_chunks(observed), sequences)

        observed = chunk_sequences(sequences, chunk_size=5)
        self.assertEqual(len(observed), 2)
        self.assertEqual(self._flatten_chunks(observed), sequences)


class CollateLasReportsTests(TestPluginBase):
    package = 'q2_dwq2.tests'

    def test_collate_two_las_reports(self):
        expected = pd.DataFrame([
          ['q1', 'r2', 100., 20, 40., 'ACACTCTCCACCCATTTGCT',
                                      'ACACTCTCCACCCATTTGCT'],
          ['q1', 'r3', 95., 20, 35., 'ACACTCTCCACCCATTTGCT',
        ...
        observed = collate_las_reports({0: part1, 1: part2})

        pdt.assert_frame_equal(observed, expected)

    def test_collate_three_las_reports(self):
        expected = pd.DataFrame([
          ['q1', 'r2', 100., 20, 40., 'ACACTCTCCACCCATTTGCT',
                                      'ACACTCTCCACCCATTTGCT'],
          ['q1', 'r3', 95., 20, 35., 'ACACTCTCCACCCATTTGCT',
        ...
        observed = collate_las_reports({0: part1, 1: part2, 2: part3})

        pdt.assert_frame_equal(observed, expected)

    def test_collate_las_reports_sorts(self):
        expected = pd.DataFrame([
          ['q1', 'r2', 100., 20, 40., 'ACACTCTCCACCCATTTGCT',
                                      'ACACTCTCCACCCATTTGCT'],
          ['q1', 'r3', 95., 20, 35., 'ACACTCTCCACCCATTTGCT',
        ...
        observed = collate_las_reports({0: part2, 1: part1})

        pdt.assert_frame_equal(observed, expected)

    def test_collate_las_reports_filters(self):
        expected = pd.DataFrame([
          ['q1', 'r2', 100., 20, 40., 'ACACTCTCCACCCATTTGCT',
                                      'ACACTCTCCACCCATTTGCT'],
          ['q2', 'r1', 100., 20, 40., 'ACACTCACCACCCAATTGCT',
        ...
        observed = collate_las_reports({0: part2, 1: part1}, n=1)
